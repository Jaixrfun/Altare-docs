...
To have voting sites listen for votes through the plugin, you need to have an open port that can be accessed by the site available. You can do this by heading into the `Network` tab on the Fractal dashboard, and press the "**Create Allocation**" button to create a port.
